...
public final class ProblemSolver implements IProblemSolver<TestCase>
{
    ...
    public String solve(TestCase instance)
    {
        ...
        int deceitResult = this.solveDeceitfulWar(this.toList(instance.getNaomi()), this.toList(instance.getKen()));
        int warResult = this.solveWar(this.toList(instance.getNaomi()), this.toList(instance.getKen()));
    ...
    private int solveWar(ArrayList<Double> naomi, ArrayList<Double> ken)
    {
        ...
        while (!naomi.isEmpty())
        {
            ...
            if (chosenNaomi > chosenKen) naomiScore++;
        }
    ...
    {
        int chosenKenIndex = -1 * Collections.binarySearch(ken, chosenNaomi) - 1;
        
        if (chosenKenIndex == ken.size()) chosenKenIndex = 0;
